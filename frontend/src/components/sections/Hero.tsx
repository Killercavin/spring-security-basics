      ...
      <div className="container-standard grid lg:grid-cols-2 gap-12 items-center min-h-[70vh]">
        ...
        <div>
          ...
          <p className="text-body text-lg leading-relaxed mb-8 max-w-lg">
            Gatelog is built on top of paper logbooks with a fast, accountable digital system.
            Every visit attempt logged, with every second timestamped and visible to the right
            people in real time.
          </p>

          <ul className="flex flex-col gap-2 mb-9">
            {[
              'Real-time dashboard, that trully reveal who is on premises with time',
              'Returning visitor auto-fill, check-in under 30 seconds'
            ].map((item, i) => (
              <li key={i} className="flex items-start gap-2.5 text-sm text-body">
                <CheckCircle2 size={16} className="text-green-mid mt-0.5 flex-shrink-0" />
                {item}
              </li>
            ))}
          </ul>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <button
              onClick={() => scrollTo('#features')}
               className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-white border text-green-deep cursor-pointer font-semibold text-sm hover:bg-white hover:bggreen-deep hover:-translate-y-0.5 transition-all duration-200"
            >
              See how it works
            </button>
          </div>
        ...
        <div className="relative">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-[0_4px_32px_rgba(13,31,22,0.08)]">
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
              <span className="font-display font-bold text-sm text-dark tracking-tight">Today's Visitors</span>
              ...
              {visitors.map((v) => (
                <div key={v.id} className="flex items-center gap-3 py-2.5">
                  ...
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0 ${
                    v.status === 'in' ? 'bg-green-light text-green-deep' : 'bg-gray-100 text-muted'
                  }`}>
            ...
            <div className="grid grid-cols-3 gap-2.5 mt-4 pt-4 border-t border-gray-100">
              {stats.map((s) => (
                <div key={s.id} className="bg-green-pale rounded-xl px-3 py-3 text-center">
                  <p className="font-display font-extrabold text-2xl text-dark leading-none mb-1">{s.value}</p>
